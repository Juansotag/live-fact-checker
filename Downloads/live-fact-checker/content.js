...
(() => {
  ...
  const STABLE_THRESHOLD = 1;   // require 1 poll stable (~300ms) — faster response
  ...
  const MAX_WINDOW_WAIT = 3000;  // after 3s of changing text, send what we have (faster response)
  const LIVE_FLUSH_INTERVAL = 4000; // for live, flush every 4s of accumulated text (capture updates quickly)
  ...
  function startMonitoring() {
    ...
    // Poll every 300ms — capture updates quickly
    pollTimer = setInterval(pollCaptions, 300);
