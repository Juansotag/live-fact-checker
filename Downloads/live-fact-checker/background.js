...
let sidePanelPort = null;  // Keep reference to side panel connection
let messageQueue = [];     // Queue messages if sidepanel not connected yet
const MAX_QUEUE = 100;     // Don't queue too many

// ── Register side panel connection to receive forwarded messages ──
chrome.runtime.onConnect.addListener((port) => {
  console.log('[BG] ✓ Connection established:', port.name);
  if (port.name === 'sidepanel-port') {
    sidePanelPort = port;
    console.log('[BG] Sidepanel connected! Flushing', messageQueue.length, 'queued messages');
    
    // Flush any queued messages
    while (messageQueue.length > 0) {
      const msg = messageQueue.shift();
      console.log('[BG] Flushing queued message:', msg.type);
      port.postMessage(msg);
    }
    
    // If connection closes, clear reference
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  switch (message.type) {
    ...
    case 'CONTEXT_UPDATE':
    case 'CONTENT_STATUS':
      // Forward these messages to the side panel
      if (sidePanelPort) {
        console.log('[BG] ✓ Forwarding to sidepanel:', message.type);
        sidePanelPort.postMessage(message);
      } else {
        // Queue if sidepanel not connected yet
        if (messageQueue.length < MAX_QUEUE) {
          console.log('[BG] ⚠ Sidepanel not connected, queueing:', message.type, `(${messageQueue.length}/${MAX_QUEUE})`);
          messageQueue.push(message);
        } else {
          console.log('[BG] ✗ Queue full, dropping message:', message.type);
        }
      }
      break;
  }
