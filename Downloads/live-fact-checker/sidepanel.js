...
(() => {
  ...
  console.log('[Sidepanel] ✓ Script loaded and initializing...');
  ...
  const state = {
    ...
    checkInterval: 10000,
    minWords: 10,  // Require ~10 words (roughly 1 sentence) before sending to Gemini
  ...
  async function init() {
    ...
    applyLanguage();
    console.log('[Sidepanel] ✓ Language applied');
    setupUI();
    console.log('[Sidepanel] ✓ UI setup');
    setupMessageListener();
    console.log('[Sidepanel] ✓ Message listener setup');
    await fetchActiveTab();
    console.log('[Sidepanel] ✓ Active tab fetched');
    await fetchPageContext();
    console.log('[Sidepanel] ✓ Page context fetched');
...
CORRECTED TEXT (MUST be similar length, same meaning):`;
  ...
  async function analyzeNow() {
    ...
    // Use overlapping chunks (30-word overlap) to avoid missing claims at boundaries
    ...
    const seenClaimTexts = new Set(); // dedup claims across overlapping chunks
      ...
      try {
        ...
        if (claims?.length) {
          for (const cl of claims) {
            ...
            // Deduplicate across overlapping chunks using normalized claim text
            const normKey = (obj.text || obj.summary).toLowerCase().replace(/\s+/g,' ').trim().substring(0,80);
            if (seenClaimTexts.has(normKey)) continue;
            seenClaimTexts.add(normKey);
  ...
  function handleTranscriptText(text) {
    ...
    // Only reject if EXACTLY duplicate (same normalized form)
    const norm = trimmed.toLowerCase().replace(/\s+/g, ' ');
    if (recentTexts.includes(norm)) {
      console.log(`[handleTranscriptText] ✗ Duplicate (exact match)`);
      return;
    }
    ...
    // Add to transcript
    const entry = { id: 't-' + Date.now(), text: trimmed };
    state.transcript.push(entry);
    state.fullText += (state.fullText ? ' ' : '') + trimmed;
    state.pendingText += (state.pendingText ? ' ' : '') + trimmed;
    state.wordCount += trimmed.split(/\s+/).length;
    
    console.log(`[handleTranscriptText] ✓ Added "${trimmed.substring(0,40)}..."`);
  ...
  // ==========================================================
  // MESSAGE LISTENER (via port communication with background)
  // ==========================================================
  function setupMessageListener() {
    console.log('[Sidepanel] Setting up message listener...');
    
    // Connect to background.js via port
    const port = chrome.runtime.connect({ name: 'sidepanel-port' });
    console.log('[Sidepanel] ✓✓✓ CONNECTED to background via port ✓✓✓');
    
    port.onMessage.addListener((msg) => {
      console.log('[Sidepanel] ✓✓✓ Message received via port:', msg.type, '✓✓✓');
      if (msg.type === 'CAPTION_UPDATE') {
        console.log(`[Sidepanel] → handleTranscriptText("${msg.text?.substring(0,40)}...")`);
        handleTranscriptText(msg.text);
      } else if (msg.type === 'CONTEXT_UPDATE' && msg.context) {
        console.log('[Sidepanel] → updating context');
        ...
        console.log('[Sidepanel] → content status:', msg.message);
        setStatus(msg.message, 'checking');
      }
    });
    
    port.onDisconnect.addListener(() => {
      console.log('[Sidepanel] ✗✗✗ Port disconnected from background ✗✗✗');
    });
  }
  ...
  function parseJSON(text) {
    ...
    try { 
      ...
      console.log('[parseJSON] ✓ Successfully parsed JSON directly');
      return result;
    } catch {}
    ...
    for (const startChar of ['{', '[']) {
      ...
      // Try from both first and last occurrences
      for (const idx of [firstStart, lastStart]) {
        ...
        if (lastEnd > idx) {
          try { 
            const jsonStr = cleaned.substring(idx, lastEnd + 1);
            const result = JSON.parse(jsonStr);
            console.log('[parseJSON] ✓ Successfully extracted JSON substring');
            return result;
          } catch (e) {
            // Try again with truncation fix: if it fails, try closing the JSON properly
            try {
              const truncated = cleaned.substring(idx);
              // Count open/close brackets to estimate how many are missing
              let openBrackets = (truncated.match(/{/g) || []).length;
              let closeBrackets = (truncated.match(/}/g) || []).length;
              let openArrays = (truncated.match(/\[/g) || []).length;
              let closeArrays = (truncated.match(/\]/g) || []).length;
              
              let toAdd = '';
              for (let i = 0; i < (openBrackets - closeBrackets); i++) toAdd += '}';
              for (let i = 0; i < (openArrays - closeArrays); i++) toAdd += ']';
              
              const completed = truncated + toAdd;
              const result = JSON.parse(completed);
              console.log('[parseJSON] ✓ Successfully parsed truncated JSON (added', toAdd, ')');
              return result;
            } catch (e2) {
              console.log('[parseJSON] Truncation fix failed:', e2.message.substring(0, 80));
            }
          }
        }
      }
    ...
    // Last resort: try to extract partial array/object items if present
    // Look for "claim": "..." patterns and try to reconstruct
    try {
      if (text.includes('"claim":')) {
        // Try to find individual objects and reconstruct as array
        const objectPattern = /\{[^{}]*"claim"[^{}]*\}/g;
        ...
        if (matches.length > 0) {
          ...
          if (objects.length > 0) {
            console.log('[parseJSON] ✓ Extracted', objects.length, 'partial objects from text');
            return objects;
          }
        }
      }
    ...
    console.log('[parseJSON] ✗ Could not extract valid JSON from:', cleaned.substring(0, 100));
    return null;
  }
  ...
  /** Detect logical sentence boundaries in auto-generated captions (without punctuation) */
  function detectSentenceBoundary(text) {
    // Coordinants & transition words that signal end of previous sentence + start of new idea
    const transitionWords = [
      /\b(?:pero|sin embargo|no obstante|sin embargo)\b/i,  // pero, sin embargo...
      /\b(?:however|yet|nevertheless|although)\b/i,
      /\b(?:entonces|así que|por lo tanto|en consecuencia)\b/i, // entonces, así que...
      /\b(?:then|therefore|consequently|as a result)\b/i,
      /\b(?:ahora|ahora bien|mientras tanto)\b/i,  // ahora, mientras tanto...
      /\b(?:now|meanwhile|at the same time)\b/i,
      /\b(?:también|además|por supuesto)\b/i,  // también, además...
      /\b(?:also|moreover|furthermore)\b/i,
      /\b(?:claro|evidentemente|obviamente)\b/i,  // claro, obviamente...
    ...
    // Scan for transition words
    for (let i = 0; i < words.length - 1; i++) {
      const phrase = words.slice(Math.max(0, i - 1), i + 2).join(' ');
      for (const transRe of transitionWords) {
        if (transRe.test(phrase)) {
          // Found a transition word — the sentence likely ends BEFORE it
          const idxBefore = words.slice(0, i).join(' ').length;
          if (idxBefore > 10 && idxBefore < text.length - 10) {
            lastGoodBoundary = idxBefore;
          }
        }
      }
    }

    // Also look for patterns like: [verb phrase] [subject] = new sentence starting
    // Simple heuristic: look for capital letter followed by more words (if auto-captions have any caps)
    const capRe = /\s([A-Z][a-z]+\s+(?:es|fue|está|son|está|fue|han|será))\s/i;
    const capMatch = capRe.exec(text);
    if (capMatch && capMatch.index > 10 && capMatch.index < text.length - 10) {
      lastGoodBoundary = Math.max(lastGoodBoundary, capMatch.index);
    }

    // If we found a good boundary, use it
    if (lastGoodBoundary > 0) {
      return [text.slice(0, lastGoodBoundary).trim(), text.slice(lastGoodBoundary).trim()];
    }

    return null; // No logical boundary found
  }

  /**
   * Cut text at the last sentence boundary (. ! ? or newline) before the end.
   * If no punctuation found, use intelligent heuristics to detect logical sentence boundaries.
   * Returns [textToProcess, remainder].
   * If no boundary found and text is long enough, returns [text, ''].
   */
  function splitAtSentenceBoundary(text) {
    // First: look for punctuation (most reliable)
    ...
    if (lastIdx > 0 && lastIdx < text.length - 5) {
      return [text.slice(0, lastIdx).trim(), text.slice(lastIdx).trim()];
    }

    // Second: try intelligent heuristic-based boundary detection (for auto-captions without punctuation)
    const heuristicBoundary = detectSentenceBoundary(text);
    if (heuristicBoundary) {
      return heuristicBoundary;
    }

    // Last resort: if text is reasonably long, send what we have (avoid stalling)
    // But don't send tiny fragments
    const words = text.split(/\s+/).length;
    if (words >= 10) {
      // Try to split at a natural place (around 60-70% through) to preserve context
      const targetIdx = Math.floor(text.length * 0.65);
      const spaceNear = text.lastIndexOf(' ', targetIdx);
      if (spaceNear > text.length * 0.4) {
        return [text.slice(0, spaceNear).trim(), text.slice(spaceNear).trim()];
      }
    }
  ...
  /** Preprocess text before sending to Gemini: normalize spaces, remove artifacts */
  function preprocessTranscript(text) {
    return text
      .replace(/\[.*?\]/g, '')          // remove [Music], [Applause], etc.
      .replace(/\(inaudible\)/gi, '')   // remove (inaudible) only
      .replace(/\(crosstalk\)/gi, '')   // remove (crosstalk) only
      .replace(/\s{2,}/g, ' ')          // collapse multiple spaces
      .replace(/(\w)-\s+(\w)/g, '$1$2') // rejoin hyphenated line-break splits
      .trim();
  }

  async function runClaimCheck(force = false) {
    if (!state.isRunning && !force) return;
    const rawPending = state.pendingText.trim();
    const wordCount = rawPending.split(/\s+/).filter(Boolean).length;
    console.log(`[runClaimCheck] pending="${rawPending.substring(0,50)}..." words=${wordCount} min=${state.minWords} force=${force}`);
    
    // Require minimum words. Force mode can send with 5+ words.
    if (wordCount < (force ? 5 : state.minWords)) {
      console.log(`[runClaimCheck] ✗ RETURN: not enough words (${wordCount} < ${force ? 5 : state.minWords})`);
      return;
    }
    ...
    if (!force && availableRequests() < 1) {
      ...
      console.log(`[runClaimCheck] ✗ RETURN: no API budget, wait ${Math.round(wait/1000)}s`);
      setStatus(`Buffering transcript (API slot in ${Math.round(wait/1000)}s)...`, 'checking');
      return; // keep text in pendingText, try next tick
    }

    // SIMPLIFIED STRATEGY: Only attempt split if we have >30 words
    // Otherwise, send accumulated text as-is to avoid creating fragmented junk
    let sendText, remainder;
    
    if (wordCount >= 30) {
      // Try to split at boundary for long texts
      const [textToSend, remainderFromSplit] = splitAtSentenceBoundary(rawPending);
      if (textToSend && textToSend.split(/\s+/).length >= 10) {
        sendText = textToSend;
        remainder = remainderFromSplit || '';
        console.log(`[runClaimCheck] ✓ Long text (${wordCount} words), using sentence boundary`);
      } else {
        // Split returned fragment <12 words; send everything instead
        sendText = rawPending;
        remainder = '';
        console.log(`[runClaimCheck] ✓ Long text (${wordCount} words), split failed, sending all`);
      }
    } else {
      // Text is 12-30 words: send all at once (don't risk fragmentation)
      sendText = rawPending;
      remainder = '';
      console.log(`[runClaimCheck] ✓ Medium text (${wordCount} words), sending all`);
    }

    if (!sendText) {
      console.log(`[runClaimCheck] ✗ RETURN: sendText is empty`);
      return;
    }
    state.pendingText = remainder;

    // Preprocess before sending to Gemini
    let text = preprocessTranscript(sendText);
    let processedWords = text.split(/\s+/).filter(Boolean).length;
    console.log(`[runClaimCheck] preprocessed: ${text.substring(0,60)}... words=${processedWords} (original: ${sendText.split(/\s+/).filter(Boolean).length})`);
    
    // If preprocessing removed too much, just use the original sendText
    if (processedWords < 8) {
      console.log(`[runClaimCheck] ⚠ Preprocessing removed too much (${processedWords} words left). Using original.`);
      text = sendText; // Use original, unprocessed text
      processedWords = sendText.split(/\s+/).filter(Boolean).length;
      console.log(`[runClaimCheck] Using original text: ${text.substring(0,60)}... words=${processedWords}`);
    }
    
    // Even after all that, require minimum words
    if (processedWords < (force ? 5 : state.minWords)) {
      console.log(`[runClaimCheck] ✗ RETURN: not enough words after preprocessing (${processedWords} < ${force ? 5 : state.minWords})`);
      state.pendingText = sendText + (remainder ? ' ' + remainder : '');
      return;
    }
    
    // Update pending text with remainder before sending
    state.pendingText = remainder + (state.pendingText ? ' ' + state.pendingText : '');

    const prevStatus = dom.statusText.textContent;
    setStatus(t('status_analyzing'), 'checking');

    try {
      console.log(`[runClaimCheck] → Calling identifyClaims with ${text.split(/\s+/).length} words: "${text.substring(0,80)}..."`);
      const claims = await identifyClaims(text);
      console.log(`[runClaimCheck] ✓ identifyClaims returned ${claims?.length || 0} claims:`, claims);
      ...
      if (claims?.length) {
        for (const cl of claims) {
          ...
          state.claims.set(id, obj);
          console.log(`[runClaimCheck] → Created claim "${obj.text.substring(0,50)}..."`);
          highlightClaimInTranscript(obj);
          updateStats();
          queueVerification(obj);
        }
      } else {
        console.log(`[runClaimCheck] ⚠ No claims returned from Gemini`);
      }
      setStatus(state.isRunning ? prevStatus : 'Stopped', state.isRunning ? 'live' : '');
    } catch (err) {
      console.error('[runClaimCheck] ✗ Error:', err);
      if (err.message.startsWith('RATE_LIMITED')) {
        console.log(`[runClaimCheck] Rate limited, waiting...`);
        setStatus(t('status_rate_limited'), 'checking');
      } else {
        console.log(`[runClaimCheck] API error: ${err.message}`);
        setStatus('API error: ' + err.message.substring(0, 80), 'error');
      }
      // Restore unprocessed text if API call failed
      state.pendingText = sendText + (remainder ? ' ' + remainder : '');
    }
  }
...
IMPORTANT — ABOUT THE TEXT FORMAT:
The transcript may contain fragments from a live/auto-generated caption system. Words may be split across lines, sentences may be incomplete at the start or end of the excerpt. Reconstruct the intended meaning when extracting claims — do NOT reject a claim just because a sentence appears slightly cut off at the edges.
...
7. For "claim": use the most complete verbatim quote you can find in the transcript. If the sentence is cut, include as much context as is present.
8. Write "summary" as a precise, testable assertion in ${langName} — complete the thought even if the transcript fragment is partial.
9. Write "searchQuery" as a specific search query to find data that confirms or denies the claim.

JSON array (or []):
[{"claim":"verbatim or near-verbatim quote from transcript","summary":"complete, testable assertion in ${langName}","searchQuery":"specific data-finding query"}]`;
    const claimSchema = {
      type: 'ARRAY',
      items: {
        type: 'OBJECT',
    ...
    const r = await callGemini(prompt, { temperature: 0.05, maxTokens: 1024, jsonMode: true, jsonSchema: claimSchema });
...
{"verdict":"TRUE|FALSE|UNCERTAIN","confidence":0.0-1.0,"explanation":"Claim says [X]. Official data from [source] shows [Y]. Therefore [verdict reasoning]","needsClarification":false,"clarificationQuestion":null}`;
  ...
  /**
   * Attempts to find claimText in a text node using a sequence-of-words approach.
   * This tolerates minor whitespace/fragment differences better than a plain regex.
   * Returns the best contiguous word-run match position if found, otherwise -1.
   */
  function findClaimInText(haystack, claimText) {
    if (!haystack || !claimText) return -1;
    // Direct substring match first (fast path)
    const idx = haystack.toLowerCase().indexOf(claimText.toLowerCase());
    if (idx !== -1) return idx;

    // Sliding-window word match: check if ≥70% of claim words appear in order
    const claimWords = claimText.toLowerCase().split(/\s+/).filter(w => w.length > 2);
    if (claimWords.length === 0) return -1;
    const haystackLower = haystack.toLowerCase();

    // Find first word position, then check subsequent words are nearby
    let cursor = 0;
    let matchStart = -1;
    let matched = 0;
    for (const w of claimWords) {
      const pos = haystackLower.indexOf(w, cursor);
      if (pos === -1) continue;
      if (matchStart === -1) matchStart = pos;
      matched++;
      cursor = pos + w.length;
    }
    if (matched >= Math.ceil(claimWords.length * 0.7) && matchStart !== -1) {
      return matchStart;
    }
    return -1;
  }
  ...
  function highlightClaimInTranscript(claim) {
    ...
    // Try both the verbatim claim text AND the summary (shorter, usually cleaner)
    const candidates = [claim.text, claim.summary].filter(Boolean);
    ...
    for (const candidateText of candidates) {
      for (const block of blocks) {
        ...
        for (const node of nodes) {
          ...
          let idx = -1;
          let matchLen = candidateText.length;
          try {
            const m = node.textContent.match(new RegExp(`(${escapeRegExp(candidateText)})`, 'i'));
            if (m) { idx = m.index; matchLen = m[1].length; }
          } catch {}

          // Fuzzy fallback for this node
          if (idx === -1) {
            idx = findClaimInText(node.textContent, candidateText);
            if (idx !== -1) {
              // Use actual matched length: from idx to end of last claim word found in text
              const claimWords = candidateText.toLowerCase().split(/\s+/).filter(w=>w.length>2);
              const lastWord = claimWords[claimWords.length - 1];
              const lastPos = node.textContent.toLowerCase().lastIndexOf(lastWord, idx + candidateText.length + 50);
              matchLen = lastPos !== -1 ? (lastPos + lastWord.length - idx) : Math.min(candidateText.length, node.textContent.length - idx);
            }
          }

          if (idx !== -1) {
            const before = node.textContent.substring(0, idx);
            const matched = node.textContent.substring(idx, idx + matchLen);
            const after = node.textContent.substring(idx + matchLen);
    ...
    // Last resort fallback: attach to best-scoring block by keyword overlap
    const claimWords = (claim.text + ' ' + claim.summary).toLowerCase().split(/\s+/).filter(w => w.length > 3);
    ...
    const threshold = Math.max(2, Math.ceil(claimWords.length * 0.4));
    if (best && bestScore >= threshold) {
      ...
      ind.textContent = ` [${claim.summary.substring(0,60)}…]`;
...
<div class="report-footer">
  ...
  // BOOT
  // ==========================================================
  console.log('[Sidepanel] BOOT: About to call init()');
  init();
})();
